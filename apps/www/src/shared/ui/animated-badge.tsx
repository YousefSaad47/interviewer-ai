...
export const AnimatedBadge = ({
  ...
  const content = (
    <motion.div
      ...
      viewport={{ once: true }}
      className="group relative flex max-w-fit items-center justify-center gap-3 rounded-full border border-border bg-white/80 px-4 py-1.5 text-foreground/75 backdrop-blur-sm transition-colors dark:bg-background/80 dark:text-zinc-300"
    >
      ...
      <ChevronRight className="ml-1 h-3.5 w-3.5 text-muted-foreground transition-transform duration-200 group-hover:translate-x-0.5" />
    </motion.div>
  );
  return (
    <>
