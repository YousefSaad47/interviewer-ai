  ...
  return (
    <div className="relative min-h-screen w-full overflow-x-hidden bg-background">
      ...
      <main className="relative mx-auto w-full max-w-[1400px] px-4 pt-24 pb-20 sm:px-6 md:px-8 lg:px-12 lg:pt-32">
        ...
        <div className="mb-6">
          <div className="flex flex-col gap-4 rounded-2xl border border-border/40 bg-card/45 p-4 shadow-xl shadow-black/5 backdrop-blur-xl md:flex-row md:items-center md:justify-between md:gap-6 dark:shadow-black/20">
            ...
            <div className="flex items-center gap-2">
              {isConnected && (
                <Button
                  ...
                  className="h-10 gap-2 rounded-xl border-border bg-card/80 text-sm font-medium hover:border-primary/30"
                >
                  {isMuted ? (
              ...
              {isConnected ? (
                <Button
                  ...
                  size="sm"
                  onClick={handleEnd}
                  className="h-10 gap-2 rounded-xl bg-red-500/10 border border-red-500/20 text-red-600 text-sm font-medium hover:bg-red-500 hover:text-white dark:text-red-400 transition-all duration-300"
                >
                ...
                <Button
                  ...
                  className="h-10 gap-2 rounded-xl bg-primary px-5 text-primary-foreground text-sm font-semibold shadow-lg shadow-primary/20 hover:bg-primary/95 hover:shadow-primary/30 disabled:opacity-50 transition-all duration-300"
                >
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_340px] xl:grid-cols-[minmax(0,1fr)_380px]">
            ...
            <Card className="relative h-[500px] overflow-hidden rounded-3xl border-border/40 bg-card/45 shadow-2xl backdrop-blur-xl sm:h-[550px] lg:h-[627px]">
              {!isConnected && status.value !== "connecting" ? (
                <div className="absolute top-1/2 left-1/2 flex w-full -translate-x-1/2 -translate-y-1/2 flex-col items-center gap-6 px-6 text-center">
                  <div className="flex h-24 w-24 items-center justify-center rounded-full border border-primary/25 bg-primary/10 text-primary shadow-2xl shadow-primary/5 transition-all hover:scale-105 duration-300">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="mx-auto max-w-md text-base text-muted-foreground">
                      ...
                      practice answering real-time technical interview questions.
                    </p>
                  </div>
                </div>
              ) : (
                    ...
                    {transcriptMessages.length === 0 ? (
                      ...
                      transcriptMessages.map((msg, i) => (
                        ...
                        >
                          ...
                          >
                            <span className="flex items-center gap-1 text-[10px] font-bold tracking-wider text-muted-foreground/80 uppercase px-1">
                              {msg.type === "user_message" ? (
                                <>
                            ...
                            <div
                              className={`rounded-2xl px-4 py-3 shadow-sm transition-all hover:shadow-md ${
                                msg.type === "user_message"
                                  ...
                                  : "rounded-tl-sm bg-muted/60 dark:bg-zinc-800/40 border border-border/40 text-foreground"
                              }`}
                            >
          ...
          <div className="w-full">
            <Card className="rounded-3xl border-border/40 bg-card/45 p-6 shadow-xl backdrop-blur-xl xl:p-7">
              <h3 className="mb-6 flex items-center gap-2 border-b border-border/40 pb-3 font-bold text-foreground text-lg sm:text-xl tracking-tight">
                ...
                {[
                  {
                    ...
                    value: questionCount,
                    style: "text-blue-600 dark:text-blue-400 bg-blue-500/10 border border-blue-500/20",
                    borderStyle: "border-l-blue-500",
                  },
                  {
                    label: "Completed",
                    value: currentQuestion,
                    style: "text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 border border-emerald-500/20",
                    borderStyle: "border-l-emerald-500",
                  },
                  {
                    label: "Remaining",
                    value: Math.max(0, questionCount - currentQuestion),
                    style: "text-cyan-600 dark:text-cyan-400 bg-cyan-500/10 border border-cyan-500/20",
                  ...
                  },
                  {
                    ...
                    borderStyle: isConnected ? "border-l-primary" : "border-l-slate-400",
                  },
                ].map((item, index) => (
                  <div
                    key={index}
                    className={`flex items-center justify-between rounded-xl border border-border/25 border-l-4 ${item.borderStyle} bg-card p-3.5 pl-4 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-300`}
                  >
                    <span className="text-muted-foreground text-sm font-semibold">
                      {item.label}
                    </span>
                    <span className={`rounded-lg px-3 py-1.5 text-sm font-bold shadow-sm ${item.style}`}>
                      {item.value}
                    </span>
                  </div>
                ))}
